    ...
    <section className='section dark:bg-dark dark:text-white' id="project" data-aos="flip-left" data-aos-duration="2000" data-aos-easing="ease-out-cubic">
      <Heading headingTitle="PORTFOLIO" />
        ...
        <div className='flex flex-row gap-x-4 pt-8'>
            <button className={`${projectStyles} ${hoverLinkTransition} ${activeProjectView === 'web' ? "text-main" : "text-lowDark dark:text-lowWhite dark:hover:text-main"}`} onClick={()=> {setActiveProjectView("web")}}>WEB DEV</button>
            <button className={`${projectStyles} ${hoverLinkTransition} ${activeProjectView === 'graphics' ? "text-main" : "text-lowDark dark:text-lowWhite dark:hover:text-main"}`} onClick={()=> {setActiveProjectView("graphics")}}>GRAPHICS DESIGN</button>
        </div>
        <section>
