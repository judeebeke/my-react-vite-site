...
const Contact = () => {
  ...
  const fetchFormData = async (formData) => {
    const response = await fetch(
      "https://jude-ebeke-default-rtdb.firebaseio.com/message.json",
      {
        method: "POST",
        cache: "no-cache",
  ...
  return (
    <section
      className="section pb-10 dark:bg-dark dark:text-white"
      id="contact"
      data-aos="fade-up"
    ...
    >
      ...
      <form
        className="w-full flex flex-col justify-center items-center lg:items-start gap-y-6"
      ...
      >
        ...
        <button
          type="submit"
          className={`${buttonStyles} ${hoverLinkTransition}`}
        >
          Send Message
        </button>
      </form>
