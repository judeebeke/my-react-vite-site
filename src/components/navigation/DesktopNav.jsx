  ...
  return (
    <nav className="desktopNav">
      <ul className="list-none flex flex-col gap-y-6 ">
        {navLinks.map((link) => {
          return (
            <li className={`${currentNav === link.navLinkText && isNavFull ? "w-auto" : "w-14 "} flex justify-center items-center h-14 p-4 text-center text-dark bg-gray hover:bg-main hover:text-white rounded-full `} key={link.navLinkText} onMouseEnter={()=>{openNavFullHandler(link.navLinkText)}} onMouseLeave={closeNavFullHandler}>
